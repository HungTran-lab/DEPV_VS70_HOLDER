...
volatile uint32_t g_stop_irq_time = 0;   // mốc thời gian phát hiện cạnh
void IRAM_ATTR onStopISR() {
  g_stop_irq = true;                    // chỉ set cờ, không làm việc nặng trong ISR
  g_stop_irq_time = millis();           // phục vụ debounce ở loop()
}
...
static void u57_all_off() { solenoids_off_only(); leds_off_only(); relay_off_only(); buzzer_off_only(); }
...
void doCheck() 
{
  ...
  int v3 = readADCstable(2, 8);
  int v4 = readADCstable(3, 8);
...
void loop() 
{
  ...
  if (g_stop_irq) {
    // đợi tối thiểu BOUNCE_MS và kiểm tra mức còn LOW
    if ((millis() - g_stop_irq_time) >= BOUNCE_MS && digitalRead(STOP_PIN) == LOW) {
      g_stop_irq = false;   // clear cờ trước khi xử lý
      abortSequenceToIdle();
    }
    // clear cờ nếu đã nhả và hết thời gian chống dội
    else if (digitalRead(STOP_PIN) == HIGH && (millis() - g_stop_irq_time) >= BOUNCE_MS) {
      g_stop_irq = false;
    }
  }

  // Read sensors (UI placeholder)
  uint16_t sen;
  if (pcf8575_read_word(I2C_ADDR_U58, sen)) u58_inputs = sen;
